...
-- if detailFlag is set, then the grosspreference settings are irrelevant
-- grosspreference is the table that records the defaulting settings for any shiftpreference row
--
...
GRANT DELETE, INSERT, SELECT, UPDATE ON TABLE grosspreference TO phpserver@localhost;


-- DELIMITER @
-- implement check constraints
-- the naming convention reflects that there may exist 1 and only 1 INSERT trigger on shiftpreference table
CREATE TRIGGER grosspreference_update BEFORE UPDATE
    ON grosspreference FOR EACH ROW BEGIN
...
-- not quite as robust, but given the likely churn in the grosspreferences, better than
